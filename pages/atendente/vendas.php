...
if (isset($_POST['vender'])) {
    ...
    $stmt = $conn->prepare('INSERT INTO Vendas (id_usuario, total) VALUES (:user, :total)');
    $stmt->execute([':user'=>$auth->user()['id'],':total'=>$total]);
    ...
    <div class="table-responsive shadow-sm rounded-4 bg-white p-3">
      <table class="table table-hover align-middle mb-0" style="min-width: 600px;">
        <thead class="table-light">
          <tr>
            <th>ID</th><th>Data</th><th>Atendente</th><th class="text-end">Total (Kz)</th>
          </tr>
        </thead>
        <tbody>
...
document.getElementById('addItem').addEventListener('click', () => {
  ...
  `;
  // ao selecionar produto, seta o hidden preco
