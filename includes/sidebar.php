    ...
    <?php foreach ($menus[$perfil] as $label => $link): ?>
      <?php if (is_array($link)): ?>
        ...
        <li class="nav-item mb-2">
          <a class="nav-link d-flex justify-content-between align-items-center text-primary fw-bold"
             data-bs-toggle="collapse" href="#<?= $submenuId ?>" role="button"
             aria-expanded="false" aria-controls="<?= $submenuId ?>">
            <span><?= htmlspecialchars($label) ?></span>
            <i class="fas fa-chevron-down small"></i>
          </a>
          <div class="collapse ms-2" id="<?= $submenuId ?>">
      ...
      <?php else: ?>
        ...
        <li class="nav-item mb-1">
          <a href="<?= $link ?>"
             class="nav-link d-flex align-items-center <?= $isActive ? 'active bg-light text-primary border-start border-3 border-primary' : 'text-dark' ?>">
            <i class="fas fa-chevron-right me-2"></i>
            <span><?= htmlspecialchars($label) ?></span>
          </a>
        </li>
...
<!-- Certifique-se de ter no footer.php ou antes de </body>: -->
